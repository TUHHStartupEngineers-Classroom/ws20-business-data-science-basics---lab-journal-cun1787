...
#There are millions of providers, 
#that offer API access for free and have good documentation about how to query their service.
#You just have to google them. You can use whatever service you want. 
#For example, you can get data about your listening history (spotify), 
#get data about flights (skyscanner) or just check the weather forecast.
library(httr)

library(tuber)
client_id <- "343220664555-a1fk5dt0s04n4t7259fta7qcnvntr3v5.apps.googleusercontent.com"
client_secret <- "24YHFldgQO43cGr3qFlh3w75"
# use the youtube oauth 
yt_oauth(app_id = client_id,
         app_secret = client_secret,
         token = '')
playlistRaw<- tuber::get_playlist_items(
  filter = c(playlist_id="PLx1-WSpW9CS1TmHWXXIfL0PdpbgFNBNtb"),
  part = "contentDetails",
  max_results = 20
)
playlistRaw%>% dplyr::glimpse(20)


#4.2
#Scrape one of the competitor websites of canyon (either https://www.rosebikes.de/
#or https://www.radon-bikes.de) and create a small database. 
#The database should contain the model names and prices for at least one category. 
#Use the selectorgadget to get a good understanding of the website structure.
...
name <- html %>% 
  html_nodes(css = ".headline-wrapper> a") %>% 
  html_text() 

price <- html %>% 
  html_nodes(css = ".buybox__prize__details") %>% 
  html_text() %>% 
  as.numeric()
rosebikes_tbl <- tibble(name,price)
#<span class="catalog-category-bikes__title-text">
#  PSYCHO PATH
#</span>
